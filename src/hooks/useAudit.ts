...
import type { AuditResponse, Recommandation } from "@/types/audit";
...
export function useAudit() {
  ...
  const runAudit = useCallback(async () => {
    ...
    try {
      ...
      if (!companyId) {
        throw new Error("No company_id found in localStorage. Please complete onboarding first.");
      }

      // Send payload matching AuditRequest schema
      ...
      setAuditData(MOCK_AUDIT_DATA as unknown as AuditResponse);
      setRecommendations(MOCK_RECOMMENDATIONS as unknown as Recommandation[]);
      setIsFallback(true);
    } finally {
      setLoading(false);
    }
